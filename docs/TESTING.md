...
_This runs `vitest` in watch mode by default (or single run in CI)._
...
_Note: This requires the local development server to be running or capable of being started by Playwright (configured in `playwright.config.js`)._
