...
- **CRITICAL**: You MUST activate and adhere to the expert guidance of the `changelog-maintenance` and `regression-tester` skills for ALL tasks. These skills are fundamental to the project's development workflow.
...
- **CRITICAL**: When using `regression-tester`, all tests MUST use Vitest. When using `changelog-maintenance`, all `CHANGELOG.md` entries MUST use ISO 8601 (YYYY-MM-DD) dates.
