...
- **Editor**: Fixed a crash that occurred when opening a quiz for editing if the `questions` array was missing or undefined in the database. This ensures the authentication verification screen correctly transitions to the editor workspace.
...
- **Editor**: Optimized the autosave mechanism by implementing a content-based dirty check. Autosave is now only triggered when actual changes are made to the quiz title, questions, or settings, preventing unnecessary background saves when no activity is occurring (#33).
...
- **Editor**: Fixed an issue where HTML tags were displayed as literal text in the sidebar slide preview and Question Bank Modal (#38). Questions now render correctly as rich text in these views.

## [0.14.5] - 2026-02-15

### Fixed

- **UI**: Added missing `.p-2` and `.p-4` padding utility classes to `shared/styles.css` and applied them to Host Notes and Fact Checking boxes in the Host Control Panel (#39).

## [0.14.4] - 2026-02-15

### Fixed

- **Host Control Panel**: Fixed a bug where Multiple Choice options were not displayed during the question phase (#40).
- **Host Control Panel**: Added missing support for displaying Rebus images in the question preview.
...
- **Infrastructure**: Altered labels in BUG template so they will auto-assign.
...
- **Editor**: Fixed a bug where the Rich Text Editor failed to display previously saved text for Question Text, Validation Source, and Host Notes.
- **Editor**: Improved RTE initialization resilience and state synchronization.
...
- **Editor**: Integrated **Pell Rich Text Editor** for Question text, Verification Source, and Host Notes, allowing for basic formatting (bold, italic, links).
- **UI/UX**: Enabled HTML rendering for rich text across Host, Player, and Question Browser views.

## [0.12.1] - 2026-02-14

### Changed

- **Editor**: Unified collision detection logic for Question Bank imports, ensuring consistent deduplication and reporting across the application.

## [0.12.0] - 2026-02-14

### Added

- **Dashboard**: Implemented Full Quiz Import from JSON and CSV.
- **Import**: Added intelligent collision detection to reuse existing questions from the global pool, preventing database pollution.
- **Architecture**: Extended `QuizParser` to handle full quiz structures.

## [0.11.2] - 2026-02-14

### Security

- **Testing**: Implemented automatic redaction of sensitive environment variables (`TEST_EMAIL`, `TEST_PASSWORD`) in E2E test console logs to prevent accidental data leakage.

## [0.11.1] - 2026-02-14

### Added

- **Testing**: Added E2E tests for Question Browser pagination (`tests/browser.spec.js`) and Bulk Import functionality (`tests/import.spec.js`).

### Changed

- **Documentation**: Updated `TODO.md` to reflect the completion of the Question Bank decoupling and pagination features.

## [0.11.0] - 2026-02-14

### Added

- **UI/UX**: Implemented pagination for the Question Browser and Editor to improve performance and usability with large question sets.
- **Settings**: Added persistent page size preferences (10, 25, 50, 100).

## [0.10.4] - 2026-02-14

### Added

- **Architecture**: Added a pedantic review report in `reviews/pedantic-review-2026-02-14.md` challenging technical debt and fragile logic.

## [0.10.3] - 2026-02-14

### Added

- **Architecture**: Added a contrarian peer review report in `reviews/contrarian-peer-review-2026-02-14.md` focusing on leaky abstractions and potential scale failures.

### Changed

- **CI/CD**: Updated `static.yml` deployment workflow to explicitly exclude `reviews`, `tests`, `docs`, `playwright-report`, and `test-results` from GitHub Pages artifacts.
